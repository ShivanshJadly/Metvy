        ...
        {costText !== undefined &&
          ...
          !Number.isNaN(Number.parseFloat(costText)) && (
            <span className="text-muted-foreground">
              ₹{(Number.parseFloat(costText) * USD_TO_INR_RATE).toFixed(4)}
            </span>
          )}
      </div>
    </div>
...
export const Context = ({ className, usage, ...props }: ContextProps) => {
  ...
  if (usage?.modelId === "gemini-2.5-pro") {
    const inputCost = usage.inputTokens ? 0.000_001_25 * usage.inputTokens : 0;
    const outputCost = usage.outputTokens ? 0.000_01 * usage.outputTokens : 0;
    usage.costUSD = {
      inputUSD: inputCost,
